...
2. **30-60초 분량**으로 편집, 씬 하나당 **최대 5초**:
   ...
   - **도입(title 씬, hook): 1.5~2.5초** — 후킹 카피 (아래 규칙 2-bis 참조)
   - 전개(body 씬들): 25-45초 — 여러 개의 5초 이하 씬으로 나누어 구성
   - 결말(comment 씬): 5-10초 — 베스트 댓글 1-2개를 각각 5초 이하 씬으로

2-bis. **후킹 씬 (필수, 첫 씬 = id 1)**:
   - 첫 씬은 **반드시 `"hook": true` 필드를 가져야 한다**
   - duration은 1.5~2.5초 안에 들어와야 시청자 이탈을 방지한다 (정치 유튜브 패턴)
   - voice_text는 한국어 8음절 이내 (1.5~2.5초 안에 끝나야 함)
   - text는 한 줄, 큰 임팩트 — 화면 중앙에 펀치 줌으로 등장
   - 다음 4개 패턴 중 1개를 사용:
     - **사실 선언형**: 인물·일자·기관을 명시. 예 "○○ 4월 회의 발언", "○○ 부장의 한 마디"
     - **질문형**: 호기심 유발. 예 "왜 이런 말이 나왔을까요?"
     - **숫자 강조**: 시간 약속. 예 "1분 정리", "30초 요약"
     - **반응 강조**: 사실 기반. 예 "이 발언, 들어보세요"
3. **TTS 친화적**으로 텍스트 변환:
...
9. **highlight_words + highlight_category (필수)**: 각 씬에서 감정적으로 핵심이 되는 단어 1-3개를 highlight_words 배열에 넣으세요. 이 단어들은 영상에서 색상으로 강조됩니다.
   - 예: text가 "월급이 200도 안 돼요"면 highlight_words: ["200", "안 돼요"]
   - 숫자, 금액, 감정 표현, 핵심 명사를 우선 선택
   - **highlight_category**: 강조 색을 정한다 (정치 유튜브 §2.3 패턴):
     · "fact"      → 노랑 — 숫자, 금액, 일자, 통계, 사실 강조 (예: "200만원", "4월 15일")
     · "criticism" → 빨강 — 비판, 문제 지적, 부정적 감정 (예: "거짓말", "위반")
     · "neutral"   → emotion 기본 색 (그 외 일반 강조, 생략 가능)
...
11. **자연스러운 발화 리듬 (필수)**: voice_text에 구두점을 활용하여 사람처럼 말하게 만드세요:
   ...
   - SSML 태그(예: <break/>)는 사용 금지(시스템이 거부함). TTS가 태그를 텍스트로 읽습니다
...
## 금지어 (후킹 카피·제목·본문에 사용 금지)

다음 단어는 클릭베이트로 분류되어 사용 시 시스템이 거부합니다:
"충격", "잠깐만요", "이거 봤어", "절대", "100%", "믿을 수 없", "경악".
사실 기반 표현으로 대체하세요. 예: "충격 발언" → "○○의 4/15 발언".
...
2-bis. **후킹 씬 (필수, 첫 씬 = id 1)**:
   ...
   - voice_text는 한국어 8음절 이내 (1.5~2.5초 안에 끝나야 함)
   - text는 한 줄, 큰 임팩트 — 화면 중앙에 펀치 줌으로 등장
   - 패턴: 사실 선언형 / 질문형 / 숫자 강조 / 반응 강조 중 1개 (예: "1분 정리", "왜 이런 말을 했을까요?")
3. **TTS 친화적**으로 텍스트 변환:
   - voice_text는 읽기 쉽게 구어체로
   - 줄임말, 신조어는 풀어서 작성
...
7. **highlight_words + highlight_category (필수)**: 각 씬에서 감정적으로 핵심이 되는 단어 1-3개를 highlight_words 배열에 넣으세요.
   - 숫자, 금액, 감정 표현, 핵심 명사를 우선 선택
   - **highlight_category**: "fact"(노랑, 숫자·일자·통계) / "criticism"(빨강, 비판) / "neutral"(emotion 색, 생략 가능)
...
9. **자연스러운 발화 리듬 (필수)**: voice_text에 구두점을 활용하여 사람처럼 말하게 만드세요:
   ...
   - 누군가에게 이야기를 설명하듯 자연스러운 템포로 만드세요
   - SSML 태그(예: <break/>)는 사용 금지(시스템이 거부함)
...
1. **구조**: title(hook) → clip → commentary → clip → commentary → ... → commentary
...
4. **"title" 씬 = 후킹 씬 (필수)**: 첫 번째 씬
   - **반드시 `"hook": true` 필드를 가져야 한다**
   - duration은 1.5~2.5초 안에 들어와야 한다 (정치 유튜브 패턴)
   - voice_text는 한국어 8음절 이내
   - text는 한 줄, 화면 중앙에 펀치 줌으로 등장
   - 패턴: 사실 선언형 / 질문형 / 숫자 강조 / 반응 강조 중 1개
     예: "○○ 4/15 본회의", "1분 정리", "이 발언 들어보세요"
...
- 시청자가 이해하기 쉬운 구어체
- highlight_words로 핵심 단어 1-3개 강조 + highlight_category 지정
  ("fact" = 노랑/숫자·일자, "criticism" = 빨강/비판, "neutral" = emotion 색 = 생략 가능)
...
{{{{
  ...
  "scenes": [
    {{{{ "id": 1, "timestamp": 0, "duration": 2, "type": "title", "hook": true,
       "text": "후킹 한 줄", "voice_text": "후킹 음성, 8음절 이내",
       "emphasis": "high", "highlight_words": ["핵심"] }}}},
    {{{{ "id": 2, "timestamp": 2, "duration": 5, "type": "clip",
       "text": "의원의 발언\\n자막", "voice_text": "",
...
IDEA_GENERATION_PROMPT = """다음 국회의원의 최근 YouTube 영상 제목들을 분석하여, 유튜브 쇼츠로 만들면 조회수가 높을 아이디어 {max_ideas}개를 생성하세요.
...
1. **클릭베이트 제목**: 시청자가 참을 수 없어 클릭하게 만드는 제목
   - "충격", "논란", "폭로", "반박" 등 임팩트 있는 단어 활용
   - 의문형, 감탄형, 반전형으로 구성
2. **훅 (hook)**: 영상 첫 3초에 시청자를 잡을 한 문장
   - 질문형 또는 충격적 사실 제시
3. **각도 (angle)**: 영상을 어떤 시각으로 접근할지 (15자 이내)
   - 예: "여야 충돌 핵심 분석", "경제정책 비판 총정리"
4. **NATV 검색 키워드**: 국회방송(NATV)에서 실제 영상을 찾을 검색어
   - 의원명 + 핵심 주제 키워드 조합, 짧게
   - 예: "나경원 연금개혁", "나경원 대정부질문"
...
{{
  "ideas": [
    {{
      "title": "🔥 {lawmaker_name} 의원의 충격 발언! 국민들이 몰랐던 진실",
      "hook": "이 발언 하나로 국회가 발칵 뒤집혔습니다",
      "angle": "여야 충돌 핵심 분석",
      "natv_keywords": "{lawmaker_name} 대정부질문"
    }}
  ]
}}
```
...
def build_idea_prompt(
    ...
    """Build the idea generation prompt from recent video titles."""
