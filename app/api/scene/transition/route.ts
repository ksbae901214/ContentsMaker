...
export async function PUT(req: NextRequest) {
  try {
    const { scene_id, transition, script_path, apply_all } = await req.json();
...
transition = json.loads(${JSON.stringify(JSON.stringify(transition))})
...
`;
    ...
    return NextResponse.json(
      { error: e.message || "Transition update failed" },
      { status: 500 }
    );
  }
}
