...
> 마지막 씬에 직접 쓰는 게 표준인데, `lint_cta` 는 top-level `cta` 블록만 본다.
> 그래서 `scene_cta_closing_warnings` 가 **자막에 `①`/`1번` 이 박힌 씬**을 CTA로
> 보고 종결을 따로 검사한다 (2026-08-18 장동혁 편에서 `"번호로 답글."` 이
> 검사 없이 통과해 영상까지 나간 뒤 추가됨).
