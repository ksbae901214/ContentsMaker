...
5. **첫 번째 narration(훅) 규칙** — narrations[0]은 반드시 실제 화자(speaker 비어 있지 않게)를 배정하고, clip 내에서 가장 대립적·충격적인 1개 발언 구간(3~5초)으로 설정할 것. 이 구간은 TTS 낭독 없이 원본 음성이 재생되며 영상의 첫인상을 결정한다.
...
- "tts_text": 음성용 **대립 서사체** 문장. 주장→반박→역공 아크를 씬마다 한 비트씩 살릴 것.
  - "~했습니다" 말미에만 의존하지 말고 아이러니·의문·감탄·반전 등 다양한 문말 허용.
  - 화자가 **바뀌는 비트**에만 "○○ 후보는"으로 시작.
  - 같은 화자가 이어지면 "그런데 / 그러자 / 반면" 연결어로 시작(이름 반복 금지).

# 나레이션 규칙
- start_sec / end_sec 는 **clip 내 상대 시각** (예: 0~4초, 4~8초 ...)
- 각 항목 길이는 2~4.5초 (한 발언=한 씬, 자막이 한 화면에 들어가게)
- 항목 수는 4~7개 (총합 22~35초 권장, 목표 30초 미만)
- text(자막)는 25자 이내 인용, tts_text(음성)는 대립 서사체 한 문장 (다양한 문말 허용)
...
- "tts_text": 음성용 **대립 서사체** 문장. 주장→반박→역공 아크를 씬마다 한 비트씩 살릴 것.
  - "~했습니다" 말미에만 의존하지 말고 아이러니·의문·감탄·반전 등 다양한 문말 허용.
  - 화자 바뀔 때만 "○○ 후보는", 이어지면 "그런데/반면" 연결어.

# 나레이션 규칙
- start_sec / end_sec 는 **0초부터 시작하는 상대 시각** (clip 개념 없음)
- 각 항목 길이는 3~4.5초 (한 비트=한 씬)
- 항목 수는 4~7개 (총합 22~35초, 목표 30초 미만)
- text(자막)는 25자 이내, tts_text(음성)는 대립 서사체 한 문장 (다양한 문말 허용)
...
- "text": 화면 자막용 **짧은 인용**(25자 이내, 화자명 제외 — 코드가 "화자: text"로 합쳐 표시).
- "tts_text": 음성용 **분석 서사체** 문장. 수치 임팩트→원인→전망 아크를 씬마다 한 비트씩 살릴 것.
  - "~했습니다" 말미에만 의존하지 말고 의문·감탄·반전 등 다양한 문말 허용.

# 나레이션 규칙
- start_sec / end_sec 는 **0초부터 시작하는 상대 시각** (clip 개념 없음)
- 각 항목 길이는 3~4.5초 (한 비트=한 씬)
- 항목 수는 4~7개 (총합 22~35초, 목표 30초 미만)
- text(자막)는 25자 이내, tts_text(음성)는 분석 서사체 한 문장 (다양한 문말 허용)
- **마지막 narration은 CTA로 마무리하지 말고 본문 마지막 멘트**. CTA는 별도 필드.
